---
layout: default
title: "Google Summer of Code 2025 - Rewriting ZIMFarm from the Ground Up"
date: 2025-09-01 14:48:49 +0100
...
<a target="_blank" href="https://kiwix.org/">Kiwix</a> rewriting the 
...
The rewrite of the ZIMFarm project spanned
...
with Python and TypeScript being the primary languages used in development.
...
as a dependency manager to pin all dependencies to a specific version. Some of the libraries were upgraded to major versions while some were replaced entirely with more feature-rich ones. The most notable replacements in this ambitious re-write were:
- Flask &rarr; FastAPI
- Marshmallow &rarr; Pydantic
- Paramiko &rarr; Cryptography

As a consequence of these upgrades and replacements, a few breaking changes were introduced 
(it was hard to keep them away). This meant the versioning of the backend API to **v2** and give 
room to leverage new features completely.

Aside from the library switches and upgrades, the rewrite introduced features including but not limited to:
- adding support for SSH keys generated using the ECDSA algorithm
- adding support for RSA keys in the PEM format
- using modern type annotations and using tools like <a href="https://github.com/microsoft/pyright" target="_blank">Pyright</a>, and
  <a href="https://docs.astral.sh/ruff/" target="_blank">Ruff</a>
 to enforce type-checking
- exposing validation rules/constraints so they can be re-used on the frontend

The workers service featured similar upgrades and API calls were updated to satisfy the new
contracts.

The frontend, initially writen in JavaScript and Vue 2 were upgraded to TypeScript and Vue 3.
Similarly, libraries were upgraded and a few more removed. I tried to keep the design largely the
same (partly because I am not good at design 😅) and only made ambitious changes in the UI were
...
Re-writing the project of this size was by no means a small feat and it was challenging as much as it was exciting.
The biggest challenges I faced (in no particular order) during
the project revolved around making Marshmallow models similar to those of Pydantic,
...
Similarly, the project featured <a href="https://docs.pydantic.dev/latest/concepts/fields/#discriminator" target="_blank">Pydantic Discriminator</a> to be able to discriminate between the
different schemas of the scrapers.
...
### Future Work
There are still some open issues to close on the project. These issues are placed in three different
categores viz:
- <a href="https://github.com/openzim/zimfarm/issues?q=is%3Aissue%20state%3Aopen%20label%3Aprio1" target="_blank">prio1</a>
- <a href="https://github.com/openzim/zimfarm/issues?q=is%3Aissue%20state%3Aopen%20label%3Aprio2" target="_blank">prio2</a>
- <a href="https://github.com/openzim/zimfarm/issues?q=is%3Aissue%20state%3Aopen%20label%3Aprio3" target="_blank">prio3</a>

### Things I Learned

Re-writing the project exposed me to more situations where I had to make decisions regarding
